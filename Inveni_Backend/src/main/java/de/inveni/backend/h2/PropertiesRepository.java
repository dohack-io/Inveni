...
public interface PropertiesRepository extends CrudRepository<Property, Long> {
    @Query(value = "FROM Property WHERE date BETWEEN :date01 AND :date02 AND LOWER(description) LIKE LOWER(CONCAT('%', :description, '%')) OR LOWER(title) LIKE LOWER(CONCAT('%', :description, '%'))")
